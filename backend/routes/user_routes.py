...
def create_user():
    ...
    if data.get('role') == 'ADMIN':
        return jsonify({'error': 'No se pueden crear más administradores. El sistema tiene un único administrador.'}), 403
...
def deactivate_user(id):
    ...
    if user.id == 1:
        return jsonify({'error': 'No se puede desactivar el administrador principal'}), 400
